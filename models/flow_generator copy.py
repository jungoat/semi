...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, emb_dim, use_attn=False):
        super().__init__()
        groups_in = math.gcd(in_ch, 8)   # in_ch와 8의 최대공약수
        groups_out = math.gcd(out_ch, 8) # out_ch와 8의 최대공약수
...
class FlowGenerator(nn.Module):
    def __init__(self, in_ch=1, base_ch=32, emb_dim=128, num_classes=12):
        ...
        # Encoder
        self.enc1 = ResBlock(in_ch, base_ch, emb_dim)
        self.enc2 = ResBlock(base_ch, base_ch*2, emb_dim)
        self.enc3 = ResBlock(base_ch*2, base_ch*4, emb_dim, use_attn=True)

        # Bottleneck
        self.bot = ResBlock(base_ch*4, base_ch*4, emb_dim, use_attn=True)

        # Decoder
        self.dec3 = ResBlock(base_ch*4 + base_ch*2, base_ch*2, emb_dim)  # 128 + 64 = 192
        self.dec2 = ResBlock(base_ch*2 + base_ch, base_ch, emb_dim)      # 64 + 32 = 96
        self.dec1 = ResBlock(base_ch + in_ch, base_ch, emb_dim)          # 32 + 1 = 33
    ...
    def forward(self, x, t, c):
        ...
        # Encoder
        x1 = self.enc1(x, emb)
        x2 = self.enc2(F.avg_pool2d(x1, 2), emb)
        x3 = self.enc3(F.avg_pool2d(x2, 2), emb)

        # Bottleneck
        b = self.bot(x3, emb)

        # Decoder
        d3 = self.dec3(torch.cat([F.interpolate(b, scale_factor=2), x2], dim=1), emb)
        d2 = self.dec2(torch.cat([F.interpolate(d3, scale_factor=2), x1], dim=1), emb)
